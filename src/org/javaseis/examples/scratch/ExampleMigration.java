...
public class ExampleMigration extends StandAloneVolumeTool {
	...
	private GridDefinition computeImageGrid(GridDefinition inputGrid, ParameterService parms) {
		...
		for (int axis = 0; axis < imageAxes.length; axis++) {
			if (axis == 0) {
				imageAxes[axis] = new AxisDefinition(AxisLabel.DEPTH, Units.METERS, DataDomain.SPACE, depthAxisLength,
						0, 1, zmin, delz);
			} else {
	...
	public boolean processVolume(ToolContext toolContext, ISeismicVolume input, ISeismicVolume output) {
		...
		for (int zindx = 0; zindx < numz; zindx++) {
			...
			/*
			 * if (debugIsOn(toolContext.parms)) { rcvr.inverseTemporal();
	...
	private void testDAEquals(DistributedArray a, DistributedArray b) {
		...
		while (dapi.hasNext()) {
			...
			Assert.assertArrayEquals("Distributed Arrays differ at position: " + Arrays.toString(position), abuff,
					bbuff, floateps);
		}
