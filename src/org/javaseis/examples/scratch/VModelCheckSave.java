 ...
 *
 */
public class VModelCheckSave extends StandAloneVolumeTool {

	private static final float S_TO_MS = 1000;
	private static final int[] FFT_ORIENTATION = PhaseShiftFFT3D.SEISMIC_FFT_ORIENTATION;

	private static final Logger LOGGER = Logger.getLogger(VModelCheckSave.class
			.getName());
	...
	private static void programTerminates() throws FileNotFoundException {
		...
		try {
			VModelCheckSave.exec(parms, new VModelCheckSave());
		} catch (SeisException e) {
			LOGGER.log(Level.SEVERE, e.getMessage(), e);
		}
	}

	private static void basicParameters(String inputFileName,
			String vModelFileName) {
	...
	public boolean processVolume(ToolContext toolContext, ISeismicVolume input,
		...
		logTimerOutput("Transform Time",
				extrapR.getTransformTime() + extrapS.getTransformTime());
		logTimerOutput("Extrapolation Time", extrapR.getExtrapolationTime()
				+ extrapS.getExtrapolationTime());
	...
	private PhaseShiftFFT3D createReceiverFFT(ToolContext toolContext,
			ISeismicVolume input) {
