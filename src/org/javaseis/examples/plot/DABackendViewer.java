...
public class DABackendViewer extends JPanel implements ActionListener, ChangeListener {
	...
	private void setUpGUI() {
		...
		if (_axisLogicalOrigins != null && _axisLogicalDeltas != null) {
			...
			int annotationEnd = (int) (_axisLogicalOrigins[_plotOrientation]
					+ _axisLogicalDeltas[_plotOrientation] * N);
	 ...
	 * this writing the DistributedArrayGlobalTraceAccessor is not able to
	 * access subtraces directly. We read the whole trace and throw most of it
	 * away.
	 */
	...
	/**
	 * Here's a static method to add a JFrame around a
	 * DistributedArrayMosaicPlot using linear scaling.
	 * 
	 * @param a
	...
	public static void showAsModalDialog(DistributedArray a, String title, int rank) {
		showAsModalDialog(a, null, null, title, rank, -1);
	}
	...
	public static void showAsModalDialog(DistributedArray a, String title, int rank, int elementOffset) {
		showAsModalDialog(a, null, null, title, rank, elementOffset);
	}
	...
	public static void showAsModalDialog(DistributedArray a, long[] axisLogicalOrigins, long[] axisLogicalDeltas,
			String title, int rank) {
		showAsModalDialog(a, axisLogicalOrigins, axisLogicalDeltas, title, rank, -1);
	}

	/**
	 * Here's a static method to add a JFrame around a
	 * DistributedArrayMosaicPlot.
	 * 
	 * @param a
	 *            the data to be plotted
	 * @param axisLogicalOrigins
	 ...
	 */
	public static void showAsModalDialog(DistributedArray a, long[] axisLogicalOrigins, long[] axisLogicalDeltas,
			String title, int rank, int elementOffset) {
		...
		if (SHOW_FRAME_VIEW) {
			...
			plot[nviews] = new DABackendViewer(a, traceAccessor, ORIENTATION_FRAME, axisLogicalOrigins,
		...
		if (SHOW_SLICE_VIEW) {
			...
			plot[nviews] = new DABackendViewer(a, traceAccessor, ORIENTATION_SLICE, axisLogicalOrigins,
					axisLogicalDeltas, elementOffset);
			nviews++;
		}
			...
			try {
				SwingUtilities.invokeAndWait(new Runnable() {
					public void run() {
						try {
							f.setVisible(true);
							BufferedImage img = ImageGenerator.createImage(f);
	...
	public static void main(String[] args) {
		...
		double denominator = 0.5
				* Math.sqrt(lengths[2] * lengths[2] + lengths[1] * lengths[1] + lengths[0] * lengths[0]);
		while (da.hasNext()) {
			da.next();
			da.getFrame(frame);
