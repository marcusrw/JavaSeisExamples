 ...
 */
public class DABackendViewer extends JPanel implements ActionListener,
		ChangeListener {
	 ...
	 * Constructor.
	 */
	private DABackendViewer(DistributedArray a,
			DistributedArrayGlobalTraceAccessor traceAccessor, int plotOrientation,
			long[] axisLogicalOrigins, long[] axisLogicalDeltas, int elementOffset) {

		if (a.getDimensions() < 3) {
			throw new IllegalArgumentException(
					"DistributedArray dimensions must be >= 3");
		}

		if (a.getElementCount() > 2) {
			throw new IllegalArgumentException(
					"DistributedArray element count must be <= 2");
		}

		if (elementOffset + 1 > a.getElementCount()) {
			throw new IllegalArgumentException("illegal elementOffset = "
					+ elementOffset);
		}
		...
		if (_rank == 0) {
			...
			_plotPanel.getTile(0, 0).addMouseMotionListener(
					new MyMouseMotionHandler("tile00"));
	...
	private void setUpGUI() {
		...
		if (_axisLogicalOrigins != null && _axisLogicalDeltas != null) {
			...
			int annotationEnd = (int) (_axisLogicalOrigins[_plotOrientation] + _axisLogicalDeltas[_plotOrientation]
					* N);
			...
			_slider = new JSlider(SwingConstants.HORIZONTAL, annotationStart,
		...
		_planeValueField = new MyIntegerTextField(_slider.getValue(),
				_slider.getMinimum(), _slider.getMaximum(), 10);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (src == MyControlPanel.getAmplitudeJTextField()) {
			...
			MyControlPanel.setScaleType(MyControlPanel.getRangeScalingButton()
					.getActionCommand());
			_plotPanel.updatePlot();
		} else if (src == MyControlPanel.getScaleFactorButton()) {
			MyControlPanel.setScaleType(MyControlPanel.getScaleFactorButton()
					.getActionCommand());
			_plotPanel.updatePlot();
	...
	public synchronized void setPlaneValue(int sliderVal) {
		...
		if (_axisLogicalOrigins == null || _axisLogicalDeltas == null) {
			...
			long index = (sliderVal - _axisLogicalOrigins[_plotOrientation])
					/ _axisLogicalDeltas[_plotOrientation];
	...
	public void setFrame(int frameIndex) {
		...
		if (VERBOSE) {
			System.out.printf(
					"DistributedArrayMosaicPlot: time for setFrame was %.2f seconds\n",
					(t1 - t0) / 1000f);
		}
	}
	...
	public void setCrossframe(int traceIndex) {
		...
		if (VERBOSE) {
			System.out
					.printf(
							"DistributedArrayMosaicPlot: time for setCrossframe was %.2f seconds\n",
							(t1 - t0) / 1000f);
		}
	}
	...
	public void setSlice(int sliceIndex) {
		...
		if (VERBOSE) {
			System.out.printf(
					"DistributedArrayMosaicPlot: time for setSlice was %.2f seconds",
					(t1 - t0) / 1000f);
		}
	}
	...
	private class MyPlotPanel extends PlotPanel {
		...
		public void updatePlot() {
			...
			_pixelsView.setClips(MyControlPanel.getClipMin(min),
					MyControlPanel.getClipMax(max));
	...
	private static class MyControlPanel extends JPanel {
		...
		private final static JRadioButton _scaleFactorButton = new JRadioButton(
				"Use amplitude scale factor:");

		private final static JRadioButton _rangeScalingButton = new JRadioButton(
				"Use clip range:");
	...
	private class MyMouseMotionHandler implements MouseMotionListener {
		...
		private void updateLocation(MouseEvent event) {
			...
			if (event.getSource().getClass() == Tile.class) {
				...
				if (xpos >= 0 && xpos < _plotData.length && ypos >= 0
						&& ypos < _plotData[xpos].length) {
					String s = String.format("(x,y,val) = (%d, %d, %f)", xpos, ypos,
							_plotData[xpos][ypos]);
	...
	public static void showAsModalDialog(DistributedArray a, String title,
		...
		showAsModalDialog(a, null, null, title, a.getParallelContext().rank(), -1,
				toolContext);
	}
	 ...
	 */
	public static void showAsModalDialog(DistributedArray a, String title,
			int rank, ToolContext toolContext) {
	 ...
	 */
	public static void showAsModalDialog(DistributedArray a, String title,
			int rank, int elementOffset, ToolContext toolContext) {
	 ...
	 *          mpi task #
	 */
	public static void showAsModalDialog(DistributedArray a,
			long[] axisLogicalOrigins, long[] axisLogicalDeltas, String title,
			int rank, ToolContext toolContext) {
		showAsModalDialog(a, axisLogicalOrigins, axisLogicalDeltas, title, rank,
				-1, toolContext);
	}
	...
	public static void showAsModalDialog(DistributedArray a,
		...
		if (SHOW_FRAME_VIEW) {
			...
			plot[nviews] = new DABackendViewer(a, traceAccessor, ORIENTATION_FRAME,
					axisLogicalOrigins, axisLogicalDeltas, elementOffset);
			nviews++;
		}
		if (SHOW_CROSSFRAME_VIEW) {
			if (VERBOSE) {
				System.out
						.println("DistributedArrayMosaicPlot: adding crossframe view");
			}

			plot[nviews] = new DABackendViewer(a, traceAccessor,
					ORIENTATION_CROSSFRAME, axisLogicalOrigins, axisLogicalDeltas,
					elementOffset);
			nviews++;
		}
		if (SHOW_SLICE_VIEW) {
			if (VERBOSE) {
				System.out
						.println("DistributedArrayMosaicPlot: adding slice view (this can be slow...)");
			}

			plot[nviews] = new DABackendViewer(a, traceAccessor, ORIENTATION_SLICE,
					axisLogicalOrigins, axisLogicalDeltas, elementOffset);
			nviews++;
		}
		...
		if (rank == 0) {
			...
			if (INCLUDE_SHAPE_INFORMATION_IN_TITLE) {
				int[] tempshape = a.getShape();
				int[] shape = tempshape;
				title += String.format("    Shape = %d %d %d", shape[0], shape[1],
						shape[2]);
			}
			...
			try {
				...
				if (toolContext == null) {
					...
					String outLocation = toolContext
							.getParameter(ToolContext.OUTPUT_FILE_SYSTEM)
							+ "//"
	...
	public class MyIntegerTextField extends JTextField {
		...
		public void setValue(int value) {
			super.setText(Integer.toString(Math.max(_minValue,
					Math.min(_maxValue, value))));
		}
	...
	public static void main(String[] args) {
		...
		int[] decompTypes = { Decomposition.NONE, Decomposition.BLOCK,
				Decomposition.BLOCK };
		long maxlen = DistributedArray.getShapeLength(3, 1, lengths);
		DistributedArray da = new DistributedArray(pc, float.class, 3, 1, lengths,
				decompTypes, maxlen);

		// Populate DA
		float[][] frame = new float[lengths[1]][lengths[0]];
		da.resetFrameIterator();
		double denominator = 0.5 * Math.sqrt(lengths[2] * lengths[2] + lengths[1]
				* lengths[1] + lengths[0] * lengths[0]);
		while (da.hasNext()) {
			da.next();
			da.getFrame(frame);
			...
			for (int j = 0; j < lengths[1]; j++) {
				...
				for (int i = 0; i < lengths[0]; i++) {
					...
					sampleSq *= sampleSq;
					frame[j][i] = (float) (1.0 - Math.sqrt(frameSq + traceSq + sampleSq)
							/ denominator);
		...
		DABackendViewer.showAsModalDialog(da, logicalOrigins, logicalDeltas,
				"Distributed Array Plot Test", pc.rank(), null);
		DABackendViewer.showAsModalDialog(da, logicalOrigins, logicalDeltas,
				"Distributed Array Plot Test", pc.rank(), null);
	}
}