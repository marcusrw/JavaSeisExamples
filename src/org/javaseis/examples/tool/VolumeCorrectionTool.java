...
public class VolumeCorrectionTool implements IVolumeTool {
	...
	private static final Logger LOGGER = Logger
			.getLogger(VolumeCorrectionTool.class.getName());

	private static final long serialVersionUID = 1L;
	private static final int[] desired_Axis_Order = new int[] { 2, 1, 0 };
	...
	public void parallelInit(IParallelContext pc, ToolState toolState)
		...
		ipio.open(inputFileName);
		ISeismicVolume outputVolume = (ISeismicVolume) toolState
				.getObject(ToolState.OUTPUT_VOLUME);
	...
	private double CalculateNewDeltaOrigin(AxisDefinition axis, int k,
			double[] data) {
		if (k == 0) {
	...
	private double CalculateNewPhysicalOrigin(AxisDefinition axis, int k,
			double[] origins) {
		if (k == 2) {
			return origins[0];
		} else if (k == 1) {
	...
	private GridDefinition updateVolumeGridDefinition(ISeismicVolume input,
		...
		LOGGER.finer("[PV updateVolumeGridDefinition] Source: "
				+ Arrays.toString(srcXYZ));

		LOGGER.finer("[PV updateVolumeGridDefinition] Reciever: "
		...
		LOGGER.fine("[PV updateVolumeGridDefinition] Reciever: "
				+ Arrays.toString(recXYZ));

		LOGGER.fine("[PV updateVolumeGridDefinition] Delta: "
		...
		for (int gridDefIndex = 0; gridDefIndex < inputAxisLengths.length; gridDefIndex++) {
			...
			physicalOAxisArray[gridDefIndex] = new AxisDefinition(
					inputAxis.getLabel(), inputAxis.getUnits(), inputAxis.getDomain(),
					inputAxis.getLength(), inputAxis.getLogicalOrigin(),
					inputAxis.getLogicalDelta(), CalculateNewPhysicalOrigin(inputAxis,
							gridDefIndex, recXYZ), CalculateNewDeltaOrigin(inputAxis,
							gridDefIndex, recXYZ2));
		}
	 ...
	 */
	@Deprecated
	private GridDefinition generateOutputGrid(ISeismicVolume input,
			double[] minPhyO, double[] Delta) {
		GridDefinition inputGrid = input.getGlobalGrid();
		long[] inputAxisLengths = inputGrid.getAxisLengths();
		...
		for (int k = 0; k < inputAxisLengths.length; k++) {
			...
			physicalOAxisArray[k] = new AxisDefinition(inputAxis.getLabel(),
					inputAxis.getUnits(), inputAxis.getDomain(), inputAxis.getLength(),
					inputAxis.getLogicalOrigin(), inputAxis.getLogicalDelta(),
					minPhyO[k], Math.abs(Delta[k]));
		}
	...
	private int[] computeAxisOrder(ISeismicVolume input) {
		...
		LOGGER.finer("[computeAxisOrder]: " + Arrays.toString(strPos));
		LOGGER.finer("[computeAxisOrder]: " + Arrays.toString(sXYZ));
		LOGGER.finer("[computeAxisOrder]: " + Arrays.toString(rXYZ));
		...
		for (int k = 1; k < 3; k++) {
			...
			LOGGER.finer("pos: " + Arrays.toString(strPos));
			LOGGER.finer("rxyz: " + Arrays.toString(rXYZ));
			LOGGER.finer("rxyz2: " + Arrays.toString(rXYZ2));
			if (Math.abs(rXYZ[0] - rXYZ2[0]) > 0.5) {
				Xindex = k;
				LOGGER.finer("Xindex is " + Xindex);
			}
			strPos[k]--;
		}
		for (int k = 1; k < 3; k++) {
		...
		LOGGER.fine("[computeAxisOrder]: Axis Order: "
	...
	private int[] computeInverse(int[] Axis_Order) {
		...
		LOGGER.fine("[computeInverse]: " + Arrays.toString(inverseOrder));
		return inverseOrder;
	}

	private void flipaxis(ISeismicVolume input, ISeismicVolume output,
			VolumePropertyService propsInput, VolumePropertyService propsOutput,
		...
		LOGGER.finer("Composed Axis 0: " + desired_Axis_Order[inverse_Order[0]]);
		LOGGER.finer("Composed Axis 1: " + desired_Axis_Order[inverse_Order[1]]);
		LOGGER.finer("Composed Axis 2: " + desired_Axis_Order[inverse_Order[2]]);
		...
		while (iti.hasNext()) {
			...
			LOGGER.finer("Input Iterator Pos: " + Arrays.toString(tracePos));
			...
			int sampleLen = (int) (inputGrid.getAxis(
					desired_Axis_Order[inverse_Order[0]]).getLength() - 1);
			int traceLen = (int) (inputGrid.getAxis(
					desired_Axis_Order[inverse_Order[1]]).getLength() - 1);
			int frameLen = (int) (inputGrid.getAxis(
					desired_Axis_Order[inverse_Order[2]]).getLength() - 1);

			double samplePhysicalOrigin = inputGrid.getAxis(
					desired_Axis_Order[inverse_Order[0]]).getPhysicalOrigin();
			double tracePhysicalOrigin = inputGrid.getAxis(
					desired_Axis_Order[inverse_Order[1]]).getPhysicalOrigin();
			double framePhyscialOrigin = inputGrid.getAxis(
					desired_Axis_Order[inverse_Order[2]]).getPhysicalOrigin();

			sampleDelta = inputGrid.getAxis(desired_Axis_Order[inverse_Order[0]])
					.getPhysicalDelta();
			traceDelta = inputGrid.getAxis(desired_Axis_Order[inverse_Order[1]])
					.getPhysicalDelta();
			frameDelta = inputGrid.getAxis(desired_Axis_Order[inverse_Order[2]])
					.getPhysicalDelta();
			...
			LOGGER.finer("Output Trace Location: "
			...
			LOGGER.finer("Before Trace Copy: " + Arrays.toString(rc));
			...
			LOGGER.finer("After Trace Copy: " + Arrays.toString(rc));
	...
	public boolean processVolume(IParallelContext pc, ToolState toolState,
		...
		LOGGER.fine("INPUT GRID: " + inputGrid.toString());
		...
		VolumePropertyService propsInput = (VolumePropertyService) input
				.getPropertyService();

		VolumePropertyService propsOutput = (VolumePropertyService) output
				.getPropertyService();

		// int[] desired_Axis_Order = new int[] { 2, 1, 0 };

		flipaxis(input, output, propsInput, propsOutput, inputGrid);

		GridDefinition outG2 = updateVolumeGridDefinition(output, toolState);

		LOGGER.fine("OUTPUT GRID: " + outG2.toString());

		setOutgoingDataStateGrid(toolState, outG2);

		LOGGER.info("Volume Coordinates Have Been Successfully Corrected.");
	...
	@Override
	public boolean outputVolume(IParallelContext pc, ToolState toolContext,
