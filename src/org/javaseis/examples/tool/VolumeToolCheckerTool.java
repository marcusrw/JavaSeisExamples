...
public class VolumeToolCheckerTool implements IVolumeTool {
	 ...
	 * coordinate service
	 * 
	 * @param input ISeismicVolume input
	 */
	...
	/**
	 * Checks the Coordinates received from the Input service against the coordinates in the Input grid
	 * @param recGrid GridDefinition receiverGrid
	 * @param input ISeismicVolume input
	 * @param axis_order The order that is expected of the input data
	 */
	public void checkCoordRecGridRec(GridDefinition recGrid, ISeismicVolume input, int[] axis_order) {		
		...
		while (iti.hasNext()){
			...
			//TODO: check that I am representing acis order right in correction service
			
			//Compute the rec from the grid at this trace position
			double X = recGrid.getAxisPhysicalOrigin(axis_order[0]) + recGrid.getAxisPhysicalDelta(axis_order[0]) * pos[axis_order[0]];
			double Y = recGrid.getAxisPhysicalOrigin(axis_order[1]) + recGrid.getAxisPhysicalDelta(axis_order[1]) * pos[axis_order[1]];
			...
			for(int i = 0; i < recXYZ.length-1; i++){
				if (Math.abs(gridRecXYZ[i] - recXYZ[i]) > 0.1){
					    throw new IllegalArgumentException(
			            "Rec Pos Change Within Volume./n" + "Position: " + Arrays.toString(pos) + "/n"
			                + "Expected: " + Arrays.toString(gridRecXYZ) + " Got: " + Arrays.toString(recXYZ));
				}
			}
		}
	}
