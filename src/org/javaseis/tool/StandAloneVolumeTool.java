...
public abstract class StandAloneVolumeTool implements IVolumeTool {
	...
	public static void exec(ParameterService parms, IVolumeTool tool)
		...
		inputFileSystem = serialToolContext
				.getParameter(ToolContext.INPUT_FILE_SYSTEM);
		if (inputFileSystem != "null") {
			ipio = new FileSystemIOService(upc, inputFileSystem);
			inputFilePath = serialToolContext
					.getParameter(ToolContext.INPUT_FILE_PATH);
			try {
				ipio.open(inputFilePath);
				serialToolContext.putFlowGlobal(ToolContext.INPUT_GRID,
		...
		GridDefinition outputGrid = (GridDefinition) serialToolContext
				.getFlowGlobal(ToolContext.OUTPUT_GRID);
		// Create or open output file if it was requested
		outputFileSystem = serialToolContext
				.getParameter(ToolContext.OUTPUT_FILE_SYSTEM);
		// If no output specified, don't use
		boolean hasOutput = false;
		if (outputGrid != null && outputFileSystem != "null") {
			IDistributedIOService opio = new FileSystemIOService(upc,
					outputFileSystem);
			outputFilePath = serialToolContext
					.getParameter(ToolContext.OUTPUT_FILE_PATH);
			String outputMode = serialToolContext
					.getParameter(ToolContext.OUTPUT_FILE_MODE);
		...
		serialToolContext.putToolGlobal(ToolContext.TOOL_CLASS,
				(Object) (tool.getClass()));
		// Now run the tool handler which calls the implementor's methods
		int ntask;
		try {
			ntask = Integer.parseInt(serialToolContext
					.getParameter(ToolContext.TASK_COUNT));
	...
	public static class StandAloneVolumeTask extends ParallelTask {
		...
		public void run() {
			...
			toolClass = (Class<IVolumeTool>) toolContext
					.getToolGlobal(ToolContext.TOOL_CLASS);
			Exception ex = null;
			try {
			...
			try {
				if (input) {
					String inputFileSystem = toolContext
							.getParameter(ToolContext.INPUT_FILE_SYSTEM);
					ipio = new FileSystemIOService(pc, inputFileSystem);
					String inputFilePath = toolContext
							.getParameter(ToolContext.INPUT_FILE_PATH);
					ipio.open(inputFilePath);
				}
				if (output) {
					String outputFileSystem = toolContext
							.getParameter(ToolContext.OUTPUT_FILE_SYSTEM);
					opio = new FileSystemIOService(pc, outputFileSystem);
					String outputFilePath = toolContext
							.getParameter(ToolContext.OUTPUT_FILE_PATH);
					opio.open(outputFilePath);
				}
			...
			if (input) {
				toolContext.putFlowLocal(ToolContext.INPUT_GRID,
						ipio.getGridDefinition());
			}
			if (output) {
				toolContext.putFlowLocal(ToolContext.OUTPUT_GRID,
			...
			if (input) {
				...
				while (ipio.hasNext()) {
					...
					boolean hasOutput = false;
					if (shot == shotNumber) {
						hasOutput = tool.processVolume(toolContext, inputVolume,
								outputVolume);
